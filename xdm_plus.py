...
class TaskManager(): 
    def __init__(self, parent) -> None:
        ...
        self.loop = asyncio.new_event_loop()  # Create a new loop
        self.download_thread = threading.Thread(target=self.download_task_manager, daemon=True)
        # Start a different thread to run downloads
    ...
    async def addQueue(self, file):  
        ...
        async with self.condition:
            await self.condition.notify_all()  # Notify that a queue was added
    
    async def download_tasks(self):
        self.is_downloading = True
        while True:
            # Wait for self.links_and_filenames queue to have a link
            async with self.condition:
            ...
            while not self.links_and_filenames.empty():
                ...
                if filename not in self.paused_downloads:  # If filename is not in paused downloads, validate filename
                        ...
                        if m3u8_extension_in_link:                            
                            ...
                            if not playlist.is_variant:
                                ...
                                for url, seg_no in zip(segments_urls, range(len(segments_urls))):
                                    async with self.segment_semaphore:  # Limit concurrent segment downloads
